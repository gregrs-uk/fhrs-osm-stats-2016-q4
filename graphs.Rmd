---
title: "Quarterly Project 2016 Q4"
output: html_document
---

The graphs below represent statistics relating to the [OpenStreetMap GB Quarterly Project 2016 Q4](https://wiki.openstreetmap.org/wiki/UK_2016_Q4_Project:_Food_Hygiene_Ratings), which involved using [Food Hygiene Rating Scheme](http://ratings.food.gov.uk) [open data](http://ratings.food.gov.uk/open-data) to enrich the map. Links between the OSM and FHRS data were created using the OSM `fhrs:id` tag, and for a match to be considered successful, the `addr:postcode` tag had to be added to the OSM entity.

The data used for the graphs below was gathered using the [FHRS/OSM comparison tool](http://gregrs.dev.openstreetmap.org/fhrs/) and relates to the time period between **10th October 2016** and **1st January 2017**. Some figures for the districts of Dumfries, Haringey and Kingston upon Thames have been approximated because of sudden changes in the FHRS data during the project.
...
mycolours <- brewer.pal(10, "Paired")
```

```{r prep-figures, echo=F}
pc.matched.old <- colSums(old[c("matched")])/colSums(old[c("total_FHRS")])*100
pc.matched.new <- colSums(new[c("matched")])/colSums(old[c("total_FHRS")])*100
pc.total.old <- pc.matched.old + 
  colSums(old[c("matched_postcode_error")])/colSums(old[c("total_FHRS")])*100
pc.total.new <- pc.matched.new + 
  colSums(new[c("matched_postcode_error")])/colSums(old[c("total_FHRS")])*100

districts.lt.5pc <- length(
  subset(new$FHRS_matched_pc + new$FHRS_pcode_err_pc,
         new$FHRS_matched_pc + new$FHRS_pcode_err_pc < 5))
districts.gt.5pc <- length(
  subset(new$FHRS_matched_pc + new$FHRS_pcode_err_pc,
         new$FHRS_matched_pc + new$FHRS_pcode_err_pc > 5))
```
...
legend(x=as.Date("2016-11-21"), y=2,
       ...
       cex=0.75, text.width = 32)
```

The percentage of FHRS establishments successfully matched (i.e. including a matched postcode) has been improved by a factor of **`r round(pc.matched.new / pc.matched.old, digits=2)`** from **`r round(pc.matched.old, digits=2)`%** to **`r round(pc.matched.new, digits=2)`%**. The percentage of FHRS establishments successfully matched (but with a mismatched or missing postcode) has been improved by a factor of **`r round(pc.total.new / pc.total.old, digits=2)`** from **`r round(pc.total.old, digits=2)`%** to **`r round(pc.total.new, digits=2)`%**.
...
As a result of the project, **`r existing.match.districts + new.match.districts`** districts out of `r existing.match.districts + new.match.districts + no.match.districts` now have at least one successfully matched FHRS establishment as opposed to **`r existing.match.districts`** districts.

## Top 10 districts by progress

```{r plot-top10-new-matches, echo=F}
par(mai=c(1.1,2.1,0.75,0.75))
...
The graph above shows the 10 districts that have made the most progress during the project in terms of the number of matches made between FHRS establishments and OSM entities. The top district was **`r rownames(top.newmatch)[1]`**, with **`r top.newmatch$matched[1]`** new matches.
...
The graph above shows the 10 districts that have made the most progress during the project in terms of the increase in the percentage of FHRS establishments matched in OSM. (This crudely takes into account the fact that districts with more establishments probably also have more mappers.) The top district was **`r rownames(top.newmatch.pc)[1]`**, with an increase of **`r round(top.newmatch.pc$FHRS_matched_pc[1], digits=1)`%** of FHRS establishments matched in OSM.
...
plot(as.Date(colnames(top.10.pc.progress)), top.10.pc.progress[1,],
     ...
     main="Top 10 districts by progress in % of FHRS establishments matched",
     col=mycolours[1])
axis.Date(1,at=dates, format="%d/%m")
for (i in 2:10) {
...
barplot(top.matches.pc.mtx[,10:1],
        ...
        legend.text=T, args.legend = list(x=52, y=11.35, cex=0.85))
```

At the end of the project, the top district in terms of the percentage of FHRS establishments matched is `r dimnames(top.matches.pc.mtx)[[2]][1]` with **`r round(top.matches.pc.mtx[c("Valid postcode"),1] + top.matches.pc.mtx[c("Mismatched postcode"),1], digits=1)`%** of FHRS establishments matched by `fhrs:id` and **`r round(top.matches.pc.mtx[c("Valid postcode"),1], digits=1)`%** of FHRS establishments matched in terms of both `fhrs:id` and `addr:postcode`.
...
At the end of the project, the top district in terms of the percentage of relevant OSM entities (i.e. those likely to be in the FHRS database) which have a non-mismatched `addr:postcode` tag (i.e. one that doesn't disagree with the FHRS one if the entity has been matched) is `r row.names(top.pcode.pc)[1]` with **`r round(top.pcode.pc[1,c("OSM_matched_or_postcode_pc")], digits=1)`%** of relevant OSM entities having a non-mismatched postcode.
...
The graph above shows the 10 districts with the highest number of mismatched `fhrs:id` tags at the end of the project. Mismatches are generally caused by the FHRS IDs of establishments being changed since they were originally added, by errors in copying, or by adding multiple `fhrs:id`s delimited by semicolons, which is [currently unsupported](https://github.com/gregrs-uk/python-fhrs-osm/issues/11) by the [FHRS/OSM comparison tool](http://gregrs.dev.openstreetmap.org/fhrs/).
...
The graph above shows the 10 districts with the highest number of missing or mismatched `addr:postcode` tags at the end of the project.
...
**`r districts.lt.5pc`** out of `r districts.lt.5pc + districts.gt.5pc` districts still have only 0--5% of FHRS establishments matched but there are some districts with a much higher percentage of matches.
...
The most common percentage of relevant OSM entities (i.e. those likely to be in the FHRS database) which have a non-mismatched `addr:postcode` tag (i.e. one that doesn't disagree with the FHRS one if the entity has been matched) is around **16%**. 
